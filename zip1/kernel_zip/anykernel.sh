...
found=$(find init.rc -type f | xargs grep -oh "import /init.moshi.rc");
if [ "$found" != 'import /init.moshi.rc' ]; then
	#append the new lines for this option at the bottom
        echo "" >> init.rc
	echo "import /init.moshi.rc" >> init.rc
fi
